  ...
  return (
    ...
    >
      <div className="flex items-start gap-2.5">
        <div className={`${config.bg} rounded-xl w-9 h-9 flex items-center justify-center flex-shrink-0 text-base`}>
          {config.emoji}
        </div>

        <div className="flex-1 min-w-0">
          <p className={`font-semibold text-stone-900 text-sm leading-tight ${item.done ? 'line-through text-stone-400' : ''}`}>
            {item.name}
          </p>
          {item.notes && (
            <p className="text-stone-500 text-xs mt-0.5 leading-snug line-clamp-2">{item.notes}</p>
          )}
